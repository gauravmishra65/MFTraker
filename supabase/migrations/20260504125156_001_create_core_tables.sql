/*
  # Create core tables for MF & Share Tracker

  1. New Tables
    - `users` - user profiles synced from auth.users
    - `stocks` - NSE/BSE stock master data
    - `mutual_funds` - mutual fund master data
    - `watchlists` - user watchlists
    - `watchlist_items` - items in watchlists (stock or MF)
    - `portfolio_transactions` - buy/sell/SIP transactions
    - `portfolio_holdings` - computed holdings (derived from transactions)
    - `alerts` - price alerts
  ...
  2. Security
    ...
    - stocks and mutual_funds are readable by authenticated users
*/
...
CREATE POLICY "Users can insert own password resets"
  ON public.password_resets FOR INSERT TO authenticated
  WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "Users can delete own password resets"
  ON public.password_resets FOR DELETE TO authenticated
  USING (user_id = auth.uid());
